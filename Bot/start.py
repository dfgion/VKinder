...
def favorite_longpoll(
    ...
    if len(info_list) > 0:
        ...
        for element in info_list:
            people_list.append(str(element[0])+' '+str(element[1])+' '+str(element[2])+' '+str(element[3]))
        VkBot.write_msg(
                user_id=user_id, 
        ...
        for event in VkBot.longpoll.listen():
            if event.type == VkBotEventType.MESSAGE_NEW:
                ...
                elif event.message['text'].lower() == 'убрать человека из списка избранных' or event.message['text'] == '2':
                    delete_user_from_favorite(main_keyboard=favorite_keyboard, people_list=people_list, user_id=user_id, menu_keyboard=menu_keyboard)
                else:
                     VkBot.write_msg(
                            user_id=user_id, 
                            message='Некорректный ответ.\n1. Вернуться назад\n2. Убрать человека из списка избранных', 
                            keyboard=favorite_keyboard)
    else:
        VkBot.write_msg(user_id=user_id, message='У вас нет людей в списке избранных', keyboard=menu_keyboard)
        main(menu_keyboard=menu_keyboard, user_id=user_id)

def delete_user_from_favorite(
        main_keyboard, 
        people_list, 
        user_id,
    ...
    for event in VkBot.longpoll.listen():
        if event.type == VkBotEventType.MESSAGE_NEW:
            ...
            elif re.search(r"[А-ЯA-Z]{1}[а-яa-z]+\s+[А-ЯA-Z]{1}[а-яa-z]+", request):
                for element in people_list:
                    if request in element:
                        psql.delete_favourite(element.split(' ')[0])
...
def registration_longpoll(
        question, 
        user_id, 
        menu_keyboard): # Запуск сессии для регистрации пользователя в БД
            ...
            if event.type == VkBotEventType.MESSAGE_NEW:
                ...
                try:
                    ...
                    if event.message['text']:
                        ...
                        VkBot.write_msg(
                                user_id=user_id, 
                                message='Введите ваш город корректно\n 1. Вернуться назад', 
                                keyboard=location_keyboard)

def black_list_longpoll(
        user_id, 
        return_keyboard, 
        menu_keyboard): # Запуск сессии для взаимодействия с черным списком
                                ...
                                if event.type == VkBotEventType.MESSAGE_NEW:
                                        ...
                                        if request.lower() == 'вернуться назад' or request == '1':
                                            ...
                                            delete_user_from_black_list(main_keyboard=return_keyboard, 
                                                                        ...
                                                                        user_id=user_id, menu_keyboard=menu_keyboard)
                                        else:
                                            VkBot.write_msg(user_id=event.message['from_id'], 
                                                            message='Выберите вариант из предложенных!\n1. Вернуться назад\n2. Убрать человека из ЧС', 
                                                            keyboard=return_keyboard)
    else:
        VkBot.write_msg(user_id=user_id, message='У вас нет людей в черном списке', keyboard=menu_keyboard)
        main(menu_keyboard=menu_keyboard, user_id=user_id)

def delete_user_from_black_list(
        menu_keyboard,
        main_keyboard, 
        people_list, 
...
def searching_question(
        search_keyboard, like_for_id = None, 
        photo_id = None, count_like = 0, 
        fav_name = None, fav_surname = None, 
	...
	for event in VkBot.longpoll.listen():
		if event.type == VkBotEventType.MESSAGE_NEW:
			...
			if 'error' in requests.get(
				...
				VkBot.write_msg(
                    user_id=event.message['from_id'], 
                    message='Привет, {}, я Бот Legacy для поиска людей\n1. Начать поиск\n2. Просмотреть черный список\n3. Просмотреть список избранных\n Для быстрых ответов можно использовать цифры'.format(VkBot.get_name(user_id=event.message['from_id'])[0]), 
                    keyboard=menu_keyboard)
		...
		if event.type == VkBotEventType.MESSAGE_NEW:
			...
			if request.lower() == 'начать поиск' or request == '1':
				...
				if psql.registr_info(event.message['from_id']) == True:
					...
					psql.registration(
                            vk_id_user=event.message['from_id'],
                            name=VkBot.get_name(user_id=event.message['from_id'])[0],
                            surname=VkBot.get_name(user_id=event.message['from_id'])[1], 
                            age=registration_longpoll(user_id=event.message['from_id'], question='age', menu_keyboard=menu_keyboard), 
                            sex=registration_longpoll(user_id=event.message['from_id'], question='sex', menu_keyboard=menu_keyboard), 
                            city=registration_longpoll(user_id=event.message['from_id'], question='city', menu_keyboard=menu_keyboard), 
