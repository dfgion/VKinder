...
def check_favourite(vk_id_user):
	...
	all_favorite = session.query(FavUser.name, FavUser.surname, FavUser.link).filter_by(
		vk_id_user=current_users_id.vk_id_user).all()
	return all_favorite
...
def check_black_list(vk_id_user):
	current_users_id = session.query(User).filter_by(vk_id_user=vk_id_user).first()
	all_black_list = session.query(BlackList.name, BlackList.surname, BlackList.link).filter_by(
		vk_id_user=current_users_id.vk_id_user).all()
